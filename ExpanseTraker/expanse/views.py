...
from .models import category, Expanse, Income, Budget
...
class CategoryDetailView(APIView):
    ...
    def get(self, request, pk):
        category_instance = category.objects.filter((Q(user=request.user)| Q(is_default=True)) & Q(id=pk) )
    ...
    def delete(self, request, pk):
        try:
            ...
            if category_instance.is_default:
                return Response({"detail": "Cannot delete a default category."}, status=status.HTTP_400_BAD_REQUEST)
            category_instance.delete()
            return Response(status=status.HTTP_204_NO_CONTENT)
        except :
            return Response({"detail": "An error occurred while trying to delete the category."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    def patch(self, request, pk):
        try:
            ...
            category_instance = category_instance.first()
            if category_instance.is_default:
                return Response({"detail": "Cannot update a default category."}, status=status.HTTP_400_BAD_REQUEST)
            serializer = self.serializer_class(category_instance, data=request.data, context={'request': request}, partial=True)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"detail": "An error occurred while trying to update the category."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ExpanseDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        if not expanse_instance:
            return Response({"detail": "Expanse not found."}, status=status.HTTP_404_NOT_FOUND)
    ...
    def put(self, request, pk):
        ...
        if not expanse_instance:
            return Response({"detail": "Expanse not found."}, status=status.HTTP_404_NOT_FOUND)
        serializer = self.serializer_class(expanse_instance, data=request.data, context={'request': request})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class IncomeDetailView(APIView):
    ...
    def put(self, request, pk):
        ...
        if not income_instance:
            return Response({"detail": "Income not found."}, status=status.HTTP_404_NOT_FOUND)
        serializer = self.serializer_class(income_instance, data=request.data, context={'request': request})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)