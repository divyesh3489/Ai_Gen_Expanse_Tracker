...
export function IncomesPage() {
  ...
  const {
    ...
    loadMore,
    refresh,
  } = useCursorPagination<Income>('/v1/expanse/incomes/', {}, { pageSize })
  ...
  const update = useMutation({
    mutationFn: (payload: {
      ...
      note?: string
      date: string
    }) => updateIncome(payload.id, payload),
    onSuccess: async () => {
      setEditingId(null)
      setAmount('')
      setNote('')
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Incomes</h1>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Track your earnings and deposits.</p>
        </div>
      ...
      <Card className="p-4">
        ...
        >
          ...
          <Input label="Amount" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="22000.00" />
          ...
          <div className="sm:col-span-2 lg:col-span-5">
            <Input label="Note (optional)" value={note} onChange={(e) => setNote(e.target.value)} placeholder="e.g. Salary" />
          </div>
        </form>
      </Card>
      ...
      <Card className="p-4">
        {initialLoading ? (
          ...
          <div className="space-y-3">
            <div className="text-sm text-rose-700 dark:text-rose-200">{errorMessage ?? 'Failed to load incomes.'}</div>
          ...
          <>
            ...
            {!hasMore && incomes.length > 0 ? (
              <p className="mt-4 text-center text-sm text-slate-500 dark:text-slate-400">
                All records loaded ({incomes.length} shown)
              </p>
            ) : null}
            ...
            {!initialLoading && loading && incomes.length > 0 ? (
